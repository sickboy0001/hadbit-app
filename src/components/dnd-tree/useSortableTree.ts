...
export const useSortableTree = ({
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (projected && over) {
      // 予測された新しい深さ (depth) と親アイテムのID (parentId) を取得
      const { depth, parentId } = projected;
      // デバッグ用にコンソールに新しい深さと親IDを出力
      console.log("handleDragEnd depth, parentId", depth, parentId);

      // 現在のツリー状態 (items) をフラット化し、それをディープコピー (JSON経由) して clonedItems を作成
      // これにより、元の items ステートを直接変更せずに、安全にアイテム情報の変更や並び替えを行える
      const clonedItems: FlattenedItems = JSON.parse(
        JSON.stringify(flattenTree(items))
      );

      // コピーされたフラットリスト (clonedItems) 内で、ドロップ先アイテム (over.id) のインデックスを検索
      const overIndex = clonedItems.findIndex(({ id }) => id === over.id);
      // 同様に、ドラッグされたアイテム (active.id) のインデックスを検索
      const activeIndex = clonedItems.findIndex(({ id }) => id === active.id);
      // デバッグ用にコンソールに取得したインデックスを出力
      console.log(
        "handleDragEnd overIndex, activeIndex",
        overIndex,
        activeIndex
      );
